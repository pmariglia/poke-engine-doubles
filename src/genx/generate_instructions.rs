...
pub fn get_instructions_from_heal(
    ...
    let target_pkmn = state.get_side(target_side_ref).get_active(&target_slot_ref);
    ...
    if percent_hit > 0.0 {
        ...
        if defender_has_substitute && !choice.flags.sound && attacker_has_infiltrator {
            ...
            if damage_dealt != 0 {
                ...
                {
                    let attacking_side = state.get_side(attacking_side_ref);
                    let attacking_index = attacking_side
                        .get_slot_immutable(attacking_slot_ref)
                        .active_index;
                    let attacking_pokemon = attacking_side.get_active(attacking_slot_ref);
                    let damage_instruction = Instruction::Damage(DamageInstruction {
...
fn run_move(
    ...
    if let Some((max_damage_dealt, _)) = damage {
        does_damage = true;
            ...
            && min_damage_dealt < defender_active.hp
        {
            ...
            let branch_chance = num_kill_rolls as f32 / 16.0;
...
fn add_end_of_turn_instructions(
    ...
    if state.sides[0].slot_a.force_switch
        || state.sides[0].slot_b.force_switch
        || state.sides[1].slot_a.force_switch
        || state.sides[1].slot_b.force_switch
    {
        return;
    }
...
fn generate_instructions_from_team_preview(
    ...
    // order doesn't matter for switches; abilities, etc. will run after all switches are done
    ...
    // so the state does not need to be modified to reflect the additional instructions
...
pub fn generate_instructions_from_move_pair(
    ...
    if s1_a_tera {
        generate_instructions_from_tera(
            ...
            SideReference::SideOne,
            &SlotReference::SlotA,
            state.sides[0].slot_a.active_index,
            &mut incoming_instructions,
        );
    }
    if s1_b_tera {
        generate_instructions_from_tera(
            ...
            SideReference::SideOne,
            &SlotReference::SlotB,
            state.sides[0].slot_b.active_index,
            &mut incoming_instructions,
        );
    }
    if s2_a_tera {
        generate_instructions_from_tera(
            ...
            SideReference::SideTwo,
            &SlotReference::SlotA,
            state.sides[1].slot_a.active_index,
            &mut incoming_instructions,
        );
    }
    if s2_b_tera {
        generate_instructions_from_tera(
            ...
            SideReference::SideTwo,
            &SlotReference::SlotB,
            state.sides[1].slot_b.active_index,
            &mut incoming_instructions,
        );
    }
    run_mega_evolutions(
    ...
    for (state_instruction, _) in state_instructions_vec.iter_mut() {
        ...
        if !(s1_a_replacing_fainted_pkmn
            ...
            || state.sides[0].slot_a.force_switch
            || state.sides[0].slot_b.force_switch
            || state.sides[1].slot_a.force_switch
            || state.sides[1].slot_b.force_switch)
        {
