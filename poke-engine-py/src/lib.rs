...
struct PyTeamPreviewFilterSide {
    ...
    forced_leads: Option<Vec<(String, String)>>,
}
...
impl PyTeamPreviewFilterSide {
    ...
    fn new(valid_pokemon: Vec<String>, forced_leads: Option<Vec<(String, String)>>) -> Self {
...
impl PyTeamPreviewFilterSide {
    // the valid_pokemon and forced_leads represent pokemon by their names
    ...
    fn to_team_preview_options(
        ...
        let forced_leads_indices = if let Some(forced_leads) = &self.forced_leads {
            let mut lead_indices = Vec::new();
            for (name1, name2) in forced_leads {
        ...
        (valid_indices, forced_leads_indices)
    }
}
