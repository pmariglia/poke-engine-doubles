...
impl MoveChoice {
    ...
    pub fn to_u8(&self) -> u8 {
        match self {
            ...
            MoveChoice::MoveTera(slot, side, mv) => {
                (*slot as u8) * 8 + (*side as u8) * 4 + (*mv as u8)
            }
    ...
    pub fn serialize(&self) -> String {
        match self {
            ...
            MoveChoice::TeamPreview(lead_index, faint_index) => {
                format!("{},{}", lead_index.serialize(), faint_index.serialize())
            }
    ...
    pub fn to_string(&self, side: &Side, attacking_slot_ref: &SlotReference) -> String {
        match self {
            ...
            MoveChoice::TeamPreview(lead_index, faint_index) => format!(
                "{},{}",
                side.pokemon[*lead_index].id, side.pokemon[*faint_index].id
            )
            .to_lowercase(),
    ...
    pub fn from_string(
        ...
        {
            let lead_idx = pkmn_id_strings.iter().position(|s| s == parts[0]).unwrap();
            let faint_idx = pkmn_id_strings.iter().position(|s| s == parts[1]).unwrap();
            return Some(MoveChoice::TeamPreview(
                PokemonIndex::deserialize(&lead_idx.to_string()),
                PokemonIndex::deserialize(&faint_idx.to_string()),
            ));
        } else {
            return None;
        }
...
impl Side {
    pub fn active_is_charging_move(&self, slot_ref: SlotReference) -> Option<PokemonMoveIndex> {
        ...
        let vs = &self.get_slot_immutable(&slot_ref).volatile_statuses;
...
impl State {
    pub fn generate_team_preview_options(
        ...
        forced_leads: Option<Vec<(PokemonIndex, PokemonIndex)>>,
        ...
        for fi in 0..n {
            for fj in (fi + 1)..n {
                ...
                let lead_pairs: Vec<(PokemonIndex, PokemonIndex)> = match &forced_leads {
                    Some(forced) => forced
                        .iter()
                        .copied()
    ...
    pub fn root_get_all_options(
        ...
        if self.team_preview {
            panic!("Should not call root_get_all_options during team preview, instead use generate_team_preview_options");
        }
...
mod tests {
    ...
    #[test]
    fn test_forced_leads_all_options_contain_forced_pair() {
        let indices = vec![
            PokemonIndex::P0,
            PokemonIndex::P1,
            PokemonIndex::P2,
    ...
    #[test]
    fn test_forced_leads_count_single_pair() {
        let indices = vec![
            PokemonIndex::P0,
            PokemonIndex::P1,
            PokemonIndex::P2,
    ...
    #[test]
    fn test_forced_leads_count_multiple_pairs() {
        let indices = vec![
            PokemonIndex::P0,
            PokemonIndex::P1,
            PokemonIndex::P2,
    ...
    #[test]
    fn test_forced_leads_skipped_when_lead_would_be_fainted() {
        let indices = vec![
            PokemonIndex::P0,
            PokemonIndex::P1,
            PokemonIndex::P2,
    ...
    #[test]
    fn test_forced_leads_together_with_reduces_indices() {
        let indices = vec![
            PokemonIndex::P0,
            PokemonIndex::P1,
            PokemonIndex::P2,
